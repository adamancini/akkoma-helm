...
- **The bundled `garage` subchart's container could never start, in any version — three stacked config bugs.** None of these were ever caught because `ct install` had never exercised `garage.enabled` before this release (see the CI fixes below).
  ...
  3. The `secrets` volume (holding the RPC secret and admin token) had no `defaultMode`, so Kubernetes mounted it at the default `0644` — world-readable. Garage refuses to start with `rpc-secret` looser than `0600` (`Error: File ... is world-readable!`). Added `defaultMode: 0400`.
