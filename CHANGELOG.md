...
non-functional before this release (the container couldn't even boot), so this
is the first release where the feature actually works, not just a routine
dependency bump.
...
- **The bundled `garage` subchart's container could never start, and `garage.enabled: true` could never finish setup, in any version — seven stacked bugs.** None of these were ever caught because `ct install` had never exercised `garage.enabled` before this release (see the CI fixes below).
  1. Garage's binary defaults to reading its config from `/etc/garage.toml` (confirmed via `garage --help` on both `v1.0.1` and `v1.3.1`), but the StatefulSet mounts the chart's ConfigMap at `/etc/garage/garage.toml` and never told the binary to look there — it crashed immediately on every boot with `IO error: No such file or directory`. Added `GARAGE_CONFIG_FILE=/etc/garage/garage.toml` to the container env.
  2. The checked-in `garage.toml` template nested `bind_addr`/`secret_file` under an `[rpc]` table, but Garage's actual schema (confirmed against the upstream configuration reference) expects `rpc_bind_addr`/`rpc_secret_file` as top-level keys, not a nested table — Garage rejected the config with `missing field rpc_bind_addr`. Fixed the template to match Garage's real schema.
  3. The `secrets` volume (holding the RPC secret and admin token) had no `defaultMode`, so Kubernetes mounted it at the default `0644`. Added `defaultMode: 0400` — but the StatefulSet's `fsGroup: 1000` (needed so the non-root container can read the volume at all) forces group-read onto Secret-volume files regardless of `defaultMode`, so the file lands at `0440` no matter what. Garage refuses to start with `rpc-secret` looser than `0600` (`Error: File ... is world-readable!`, though the file has no "other" permissions at all — group-read alone trips Garage's check).
  4. Since `defaultMode` can't reach a strict `0600` under `fsGroup`, and the secret is already access-controlled by the Secret API/RBAC (not actually readable by other users/processes on the node), set `GARAGE_ALLOW_WORLD_READABLE_SECRETS=true` — Garage's own documented escape hatch for exactly this fsGroup-vs-permission-check conflict.
  5. The auto-generated `rpc-secret` used `randAlphaNum`, which produces mixed-case alphanumeric output, but Garage requires the RPC secret to be exactly 32 bytes of hex (`Error: Invalid RPC secret key: expected 32 bytes of random hex`). `garage.resolveSecret` now has a `"hex"` format mode (used only for `rpc-secret`) that hashes the random input with `sha256sum`, which always yields exactly 64 lowercase hex characters.
  6. `job-garage-setup`'s container runs `apk add --no-cache curl jq` at runtime to provision Garage's S3 credentials, but it was configured with both `runAsNonRoot`/`runAsUser: 1000` and `readOnlyRootFilesystem: true` — `apk` cannot install packages under either constraint, so the container crashed silently on every run (its own `> /dev/null 2>&1` swallowed the actual error). Now runs as root with a writable rootfs, scoped to this one short-lived post-install hook; `allowPrivilegeEscalation: false` and dropping all capabilities are kept.
  7. Once the container could actually run, cluster-layout assignment still failed: the script hardcoded `"capacity": 1` in the layout role, but Garage rejects capacities too small to divide into its fixed 256 partitions (`Internal error: The storage capacity of the cluster is too small`) — confirmed by exercising Garage's admin API directly against a local `v1.3.1` instance. `capacity` is now derived from `garage.persistence.data.size` (e.g. `10Gi` → `10737418240` bytes), verified end-to-end (including idempotent re-runs) against a live Garage instance before landing. The `layout/apply` call also now surfaces its response and HTTP status instead of failing silently under `curl -sf`, so any future admin-API surprise won't be a silent, contentless crash again.
- **`garage.enabled: true` installs could deadlock and never become Ready under `helm install --wait`** (and therefore under ArgoCD/Flux sync-wave health checks too). The akkoma Deployment reads S3 credentials from a Secret that's only created by `job-garage-setup`, a post-install hook — but Helm's `--wait` blocks the *entire install* on Deployment readiness before running post-install hooks at all, so the hook that would create the missing Secret could never run. The Deployment's S3 env vars are now `optional` when `garage.enabled`, so the pod can reach Ready without them, and `job-garage-setup` now triggers a rollout restart of the Deployment right after it provisions the real credentials, so they still get picked up automatically on first install. This was never caught before because `ct install` had never gotten this far (see the CI fixes below) — this specific scenario had never actually been exercised.
...
- `charts/akkoma/ci/external-db-values.yaml` simulates a pre-existing external database and references a `pg-password` secret that's expected to already exist out-of-band, but the CI job never created it, and the scenario's `postgres.example.com` host doesn't resolve to anything in the ephemeral kind cluster either — both issues were previously masked by the CNPG gap above, since the job never got far enough to hit them. `ct install` now runs against a fixed `ct-akkoma` namespace (via `--namespace`) pre-seeded with the secret and a real throwaway `postgres:15-alpine` instance (with the extensions Akkoma requires), and the install's `--helm-extra-set-args` overrides `postgresql.external.host` to point at it — the checked-in values file itself is untouched.
- `charts/akkoma/ci/external-secrets-values.yaml` simulates a pre-existing, externally-managed app secret (`my-akkoma-secrets`) that the CI job never created either — same class of bug as the two above, masked until the job got this far. Now pre-seeded in the `ct-akkoma` namespace alongside the other fixtures.
- `charts/akkoma/ci/full-values.yaml` and `metrics-values.yaml` enable `metrics.serviceMonitor`, which renders a `ServiceMonitor` (`monitoring.coreos.com/v1`), but that CRD was never installed either — same "no matches for kind" failure mode as the CNPG gap, masked until now. Installed just the `ServiceMonitor` CRD (pinned to prometheus-operator `v0.93.0`); the full operator/controller isn't needed since nothing needs to reconcile the resource for the install to succeed.

## [0.5.0] - 2026-08-06

Minor version bump rather than a patch, despite most of this being scan
remediation — the UID renumbering below is a breaking change for existing
installations.
