...
- **The bundled `garage` subchart's container could never start, in any version.** Garage's binary defaults to reading its config from `/etc/garage.toml` (confirmed via `garage --help` on both `v1.0.1` and `v1.3.1`), but the StatefulSet mounts the chart's ConfigMap at `/etc/garage/garage.toml` and never told the binary to look there — it crashed immediately on every boot with `IO error: No such file or directory`. Added `GARAGE_CONFIG_FILE=/etc/garage/garage.toml` to the container env. This predates the garage image bump in this release; it had just never been exercised by CI (see below).
