...
- **The bundled `garage` subchart's container could never start, in any version — four stacked config bugs.** None of these were ever caught because `ct install` had never exercised `garage.enabled` before this release (see the CI fixes below).
